...
#define L1 10.0 // İlk kol uzunluğu
#define L2 8.0 // İkinci kol uzunluğu -gripper merkezine olan uzaklık.
#define mm_per_step // Yukseklik motorunun bir adım başına katettiği yol. 
#define MOTOR_MAX_Z // düşey eksende maksimum derinlik.
#define tekil_gorev_suresi
#define motor3_guvenlik_suresi
#define gripper_delay
//Kutu yerleri
#define RED_BOX_1_X
#define RED_BOX_1_Y
...
double x, y
int colorVal, floor, task_x, task_y;
double kutu_alma_yuksekligi = MOTOR_MAX_Z - 6.5
    ...
    for (int ev_indexi = 0; ev_indexi < numHouses; ev_indexi++) {
      if (evler[ev_indexi].floor >= currentFloor) {
        ...
        switch (evler[ev_indexi].colorName) {
          ...
          default:
            // Tanımlanan renklerden herhangi birine uymayan renk için yapılacak işlemler
        }
      } else {
        // Ev hesaba dahil edilmeyecek
        // İlgili işlemler burada yapılabilir, örneğin atlanan evin işlemi yapılabilir
      }
      Task(a,b,x,y)
    }
  }
...
double acidan_adim(double aci) {
  int adim = aci * (360.0 / PI) / MOTOR_ANGLE_PER_STEP
  return adim
}
...
// 1.adim kinematik motorlarını ayarla

// a,b ye git, c yi ayarla, gripperi kapat,
// -c ye kadar git, -b ve - a kadar git.
// orijindesin
// x,y ye git 
// z yi tekrar ayarla
// gripperi aç
// -z ye git , -x ve -y ye git 
// tekrar orijisndesin 

// c bütün kutular için: kutu_alma_yuksekligi
...
void Task(double a, double b, double x, double y) {
  double aci1, double aci2 = inverse_kinematics(a,b)
  ...
  delay(tekil_gorev_suresi)
  //kutu orijinde
  double aci1, double aci2 = inverse_kinematics(x,y)
  int adim1 = acidan_adim(aci1)
  int adim2 = acidan_adim(aci2)
